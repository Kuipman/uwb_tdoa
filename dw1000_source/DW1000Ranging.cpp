...
void DW1000RangingClass::loop() {
	...
	if(time-timer > _timerDelay) {
		// Serial.println("you're in the timer section");
		timer = time;
		timerTick();
	}
	if(_sentAck) {
		...
		/**
		 * @brief For two-way ranging the anchor receives a poll, returns a poll response (POLL_ACK), and receives a final transmission.
		 * 		So, here our sent acknowledgement is only needed for the anchor when a poll acknowledgement is sent
		 * 
		 */
		if(_type == ANCHOR) {
