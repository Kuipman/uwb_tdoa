...
byte 	DW1000Class::_deviceID[LEN_CHAN_CTRL];
...
//Sets new TX/RX mode, pulse frequency, channel, etc
//TODO not everything here is actually needed, especially with the commitconfiguration workaround.
//	Cut the fat.
void DW1000Class::setDeviceSettings_OWR()
{
	if(_deviceMode == TX_MODE) {
		...
		//this will likely change later when we want to try out data transfer
		useExtendedFrameLength(false);
		//not sure why this is false, I'll leave this for now
		useSmartPower(false);
		//@todo what is this?
		suppressFrameCheck(false);
		//Global frame filtering -- this is necessary for OWR
		//@todo function that sets up frame filtering to accept particular frames over others
		//		(i.e. the tag does not need the sync info the base stations are sending to each other)
		//@todo function that builds the respective frames to pass through frame filtering depending on their functionality
		...
		*/
		...
		//default interrupts from default settings -- leaving alone for now
		...
		//Note this mode is 110kbps, not suitable for data transfer but great for ranging reliability (or at least it should)
	...
	//finally, ensures all changes in host are written to the DW1000
	commitConfiguration();
}

/**
 * @brief Turns on right 4 LEDs and has them blink intermittently
 * 			- will eventually be used to activate LEDs in general for use with RX/TX operations
 * 
 * @todo DIDNT WORK -- need to try something else
 */
void DW1000Class::activateLEDs()
{
	//enableLedBlinking();    //sets BLNKEN bit to ON     //DEBUG : see if this is needed when testing, may already be activated in setup
	enableDebounceClock();
	setGPIOMode(6, 1);
	setGPIOMode(8, 1);
	setGPIOMode(10, 1);
	setGPIOMode(12, 1);
	//set BLKNEN bit to ON, BLNKNOW bits 16 - 19 to ON so LEDs are manually activated
	byte pmscledc[LEN_PMSC_LEDC];
	memset(pmscledc, 0, LEN_PMSC_LEDC);    //sets all bits in pmscledc to 0
	readBytes(PMSC, PMSC_LEDC_SUB, pmscledc, LEN_PMSC_LEDC);
	setBit(pmscledc, LEN_PMSC_LEDC, BLNKEN, 1);    //BLNKEN bit to ON
	//TODO find a simpler way to change four bits
	setBit(pmscledc, LEN_PMSC_LEDC, 16, 1);
	setBit(pmscledc, LEN_PMSC_LEDC, 17, 1);
	setBit(pmscledc, LEN_PMSC_LEDC, 18, 1);
	setBit(pmscledc, LEN_PMSC_LEDC, 19, 1);
	writeBytes(PMSC, PMSC_LEDC_SUB, pmscledc, LEN_PMSC_LEDC);     //writes new byte values to PMSC LED Control Register
}

/**
 * @brief Activates GPIOs 0-3, which operate the 4 LEDs associated with the DW1000
 * 
 */

void DW1000Class::activateLEDsV2()
{
	byte gpio_leds[LEN_GPIO_MODE];
}
