...
// messages used in the two-way ranging protocol
#define POLL 0
#define POLL_ACK 1
#define RANGE 2
...
// messages used in the one-way ranging protocol
#define START_SYNC 0
#define END_SYNC 1             //Tag sends to RBS to signal end of TWR (manually, when ready for takeoff)
#define BS_TIMESYNC 2       
#define TAG_LOCALIZE 3
#define BS_TIMESYNC_FAILED 254
#define TAG_LOCALIZE_FAILED 255
...
public:
	...
	static void    startAsRBSAnchor(char address[], const byte mode[], const byte channel = DW1000.CHANNEL_5, const bool randomShortAddress = true);
	...
	//debug getters
	static uint8_t getCurrentRSTPin();
	static uint8_t getCurrentIRQPin();
	static uint8_t getCurrentSSPin();
	
	//ranging functions
	static int16_t detectMessageType(byte datas[]); // TODO check return type
	static void loop();
	static void loop_OneWay();
	...
	//Kuipers Functions
	static void setOneWayRangingMode(bool isOneWay);    //determines whether we're using one-way or two-way ranging

	static void activateModuleLEDs();
	static void RangingTest1();
	static void RangingTest2();
...
private:
	...
	static boolean _isOneWayRanging;
	//One-Way Ranging Start State
	static volatile boolean _startSync;
	// message flow state
	...
	//for one-way ranging protocol (RBS Anchor)
	static void RBStransmitInit();
	static void RBStransmitStartSync();

	//for one-way ranging protocol (ALL)
	static void setVars_OWR(int);
	static void timeSyncLoop();

	//for two-way ranging protocole (ANCHOR)
	static void transmitInit();                                        //this clears any TX flags
	static void transmit(byte datas[]);                                //transmits the contents of datas[]
	static void transmit(byte datas[], DW1000Time time);               //transmits contents of datas[] after delay time
	static void transmitBlink();                                       //generates a blink frame and sends it out
	...
	//for two-way ranging protocole (TAG)
