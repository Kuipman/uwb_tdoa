...
//Will this be one-way ranging?
boolean DW1000RangingClass::_isOneWayRanging = false;

//One-Way Ranging start state for non-RBS devices
volatile boolean DW1000RangingClass::_startSync = false;
...
void DW1000RangingClass::startAsRBSAnchor(char address[], const byte mode[], const byte channel, const bool randomShortAddress) {
	//save the address
	DW1000.convertToByte(address, _currentAddress);
	//write the address to the DW1000 chip and print to serial monitor
	DW1000.setEUI(address);
	Serial.print("Device Address: ");
	Serial.println(address);
	//set the host-tracked short address depending on the received parameters
	/*
	if (randomShortAddress) {
		//we need to define a random short address:
		randomSeed(analogRead(0));
	...
	*/
	...
	//RBS sends the Start Sync, so we set this pre-emptively to true
	_startSync = true;
	
	Serial.println("### RBS ANCHOR ###");
...
void DW1000RangingClass::loop() {
	...
	if(_sentAck) {
		//A sentAck is received (interrupt) from the DW1000, which means the DW1000 just finished sending a frame. We analyze the type of frame and
		//perform respective steps to fascilitate Two-Way Ranging
		_sentAck = false;

		if(DEBUG)
		{
			Serial.println("_sentAck received from DW1000");
			//delay(1000);               //this delay needs to be across the board for all devices, otherwise Two Way won't properly sync
		}

		//message type can be POLL (from Tag), POLL_ACK (from Anchor), or RANGE (from Tag)
		int messageType = detectMessageType(data);
		if(messageType != POLL_ACK && messageType != POLL && messageType != RANGE)
			return;
		
		//At this point the sent message is one of the three mentioned types. We launch the ranging protocole when a message was sent

		//if the DW1000 is an anchor, only the POLL_ACK is relevant for a sent message.
		//Here, the host determines the short address of the device the DW1000 sent to, then stores the timestamp of the transmission (into )
		if(_type == ANCHOR) {
	...
	// delay(1000);
	//DW1000.checkDeviceMode();     //debug - prints current device mode
}

/**
 * @brief Loop for One-Way Ranging w/Tag as receive-only
 * 
 */
void DW1000RangingClass::loop_OneWay() {
	//Just as with the two-way loop we first check for a reset
	checkForReset();
	uint32_t time = millis(); // TODO other name - too close to "timer"
	if(time-timer > _timerDelay) {
		timer = time;
	...
	//One-way Ranging sent acknowledgement -- triggered by an interrupt upon message transmission
	if(_sentAck) {
		_sentAck = false;

		// checks the type of message -- if none of the above (no message) ends the function
		int messageType = detectMessageType(data);
		if(messageType != POLL_ACK && messageType != POLL && messageType != RANGE)
			return;
		...
		/**
		 * @brief For the Anchor it sends two different transmissions
		 * 		- Timing information to other anchors
		 * 		- Ranging information to the tag
		 * @todo Separate types of transmissions for anchors
		 * 
		 */
		if(_type == ANCHOR) {
			if(messageType == POLL_ACK) {
		...
		/**
		 * @brief Tag is receive only
		 * @todo Make _sentAck for Tag obsolete
		 */
		else if(_type == TAG) {
			return;                     //Makes _sentAck obsolete for tag
...
/* ###########################################################################
 * #### Methods for One-Way Ranging   ######################################
 * ######################################################################### */

//void DW1000RangingClass::RBStransmitInit() {}

//void DW1000RangingClass::RBStransmitStartSync() {}

/**
 * @brief Task 1 of one-way ranging algorithm. Engaged after transmission
 * 		initialization and the StartSync has been sent by the RBS/received by
 * 		everyone else.
 * Upon startup and StartSync:
 * 			1) Node waits an initial delay time d, then transmits
 * 				at the rate of Tp (periodic signal transmission) (d, Tp set in RBStransmitInit())
 * 			2) During initial delay, node remains in IDLE. After delay and initial transmission, node switches
 * 				to RX whenever not in TX.
 * 			3) Upon first transmission, the node's local time is set to 0 (DW1000 Sync Input pin to HIGH)
 * Sync process:
 * 			1) For TX, a frame is sent with the following info:
 * 				- sender MAC Address (or ID)
 * 				- p, the sequential packet number (pTp)
 * 				- timestamp of packet ToE (the exact moment the frame SFD passes through the transmitter + antenna delay)
 * 			2) For RX, a frame from another node is received:
 * 				- ToE is recorded and ToA is locally-estimated. Both are stored as an object in an array (according to sender ID)
 * 				- Frames are received from every node until each node has transmitted Np amount of times
 * 				- EstimateTime() called to process each 
 * 
 */
static void timeSyncLoop()
{
...
void DW1000RangingClass::setOneWayRangingMode(bool isOneWay)
{
	_isOneWayRanging = isOneWay;
}

void DW1000RangingClass::activateModuleLEDs()
{
	DW1000.activateLEDs();
}

void DW1000RangingClass::RangingTest1()
{
	char string[25];
	transmitBlink();   //this builds the Blink frame and transmits automatically
	Serial.print("#### One_Way Ranging Set to: "); Serial.print(_isOneWayRanging); Serial.println(" #####");
	Serial.println("Blink message transmitted:");
	Serial.print("	Current Address:       ");
	sprintf(string, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
						_currentAddress[0], _currentAddress[1],_currentAddress[2],_currentAddress[3],_currentAddress[4],_currentAddress[5],_currentAddress[6],_currentAddress[7]);
	Serial.println(string);
	memset(string, 0, 25);
	Serial.print("	Current Short Address: ");
	sprintf(string, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
						_currentShortAddress[0], _currentShortAddress[1], _currentShortAddress[2], _currentShortAddress[3], _currentShortAddress[4], _currentShortAddress[5], _currentShortAddress[6], _currentShortAddress[7]);
	Serial.println(string);
}

void DW1000RangingClass::RangingTest2()
{
	char string[60];
